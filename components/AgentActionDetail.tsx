...
/** Helper: pendekkan alamat wallet agar tidak overflow */
...
export default function AgentActionDetail({
    ...
    const displayWallet = wallet ? shortenAddress(wallet) : "0x...";
    const recipient = action?.recipient || "—";
    const amount = action?.amount || "—";
    const token = action?.token || "USDT";

    // Copy state untuk recipient dan wallet
        ...
        <div className="flex min-h-screen flex-col bg-transparent">
            <header className="flex h-16 w-full items-center justify-between border-b border-white/5 bg-black/20 px-6 backdrop-blur-md">
                <div className="flex items-center gap-3 min-w-0">
                    ...
                    <span className="hidden sm:flex items-center gap-1.5 rounded-full border border-white/10 bg-black/40 px-3 py-1 font-mono text-[10px] tracking-widest text-accent uppercase ml-2 shrink-0">
                        <span className="inline-block h-1.5 w-1.5 rounded-full bg-accent animate-pulse" /> {network}
                    </span>

                    {/* Wallet chip: shortened + copy button */}
                    {wallet && (
                        <button
                            type="button"
                            onClick={() => copyToClipboard(wallet, "wallet")}
                            title={`Klik untuk copy: ${wallet}`}
                            aria-label="Copy wallet address"
                            className="group flex items-center gap-2 rounded-full border border-white/10 bg-black/40 px-3 py-1 font-mono text-xs text-gray-300 transition-all hover:border-accent/40 hover:bg-accent/5 shrink-0"
                        >
                    ...
                    <div className="mx-auto max-w-4xl relative">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-4 mb-10 bg-gradient-to-r from-accent/5 to-transparent p-6 rounded-2xl border border-white/5">
                            <div>
                                <h1 className="font-display text-4xl font-light tracking-tight text-white">
                                    Agent <span className="font-semibold text-transparent bg-clip-text bg-gradient-to-r from-accent to-purple-400">Action</span>
                                </h1>
                                <p className="mt-2 text-sm text-gray-400 font-light">
                                    Review the structured action generated from your natural language intent.
                                </p>
                            </div>
                        ...
                        <div className="grid gap-6 md:grid-cols-2 relative z-10">
                            ...
                            <div className="glass-panel rounded-2xl p-6 transition-all hover:border-white/20 hover:bg-white/5 group relative overflow-hidden">
                                <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                                    <i className="bi bi-person text-6xl text-white" />
                                </div>
                                ...
                                <div className="rounded-xl border border-white/5 bg-black/40 px-5 py-6 flex items-center justify-center min-h-[120px] shadow-inner relative z-10 overflow-hidden">
                                    <p
                                        className={`font-sans italic text-gray-200 font-light leading-relaxed text-center break-all max-w-full ${userIntent.length > 100 ? "text-sm sm:text-base" : "text-base sm:text-lg"
                                            }`}
                                    >
                                        &quot;{userIntent}&quot;
                                    </p>
                                </div>
                                <p className="mt-5 text-[11px] text-muted/70 font-sans px-1">
                                    This is the original prompt parsed directly from your input.
                                </p>
                            </div>
                            ...
                            <div className="glass-panel rounded-2xl p-6 border-accent/30 bg-gradient-to-b from-accent/5 to-transparent relative shadow-[0_4px_25px_rgba(56,189,248,0.05)]">
                                <div className="flex items-center justify-between mb-5">
                                    ...
                                    <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2 py-0.5 font-mono text-[9px] tracking-widest font-bold text-emerald-400">
                                        EXECUTION READY
                                    </span>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="rounded-xl border border-white/5 bg-black/40 px-4 py-3 shadow-inner">
                                        <div className="mb-1 flex items-center justify-between gap-2">
                                            ...
                                            >
                                                <i
                                                    className={`bi ${copiedField === "recipient" ? "bi-check2 text-emerald-400" : "bi-clipboard"
                                                        } text-[11px]`}
                                                />
                                            </button>
                                        </div>
                                ...
                                <div className="mt-4 rounded-xl border border-accent/20 bg-black/60 px-5 py-4 font-mono text-xs shadow-inner">
                                    ...
                                    <p className="text-accent/90 break-all leading-relaxed">
                                        <span className="text-purple-400">transfer</span>(recipient:{" "}
                                        <span className="text-amber-200">{recipient}</span>, amount:{" "}
                                    ...
                                    <div className="flex items-center justify-between rounded-xl border border-white/5 bg-black/20 px-5 py-4 transition-colors hover:bg-white/5">
                                        <span className="flex items-center gap-3 font-mono text-sm text-gray-200 truncate pr-4">
                                            ...
                                            <span className="truncate" title={recipient}>
                                                Recipient: {shortenAddress(recipient, 8, 6)}
                                            </span>
                                        </span>
                                        <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 font-mono text-[9px] font-bold text-emerald-400 uppercase tracking-widest shrink-0">
                                            Detected
                                        </span>
                                    </div>
                                    <div className="flex items-center justify-between rounded-xl border border-white/5 bg-black/20 px-5 py-4 transition-colors hover:bg-white/5">
                        ...
                        <div className="mt-8 flex items-center justify-center gap-4 sm:gap-6 glass-panel rounded-2xl px-6 py-8 relative z-10">
                            <div className="flex flex-col items-center gap-3 group">
                                ...
                                <span className="font-mono text-[10px] tracking-widest font-bold text-muted/70 uppercase">
                                    User Intent
                                </span>
                            </div>
                            ...
                            <div className="flex flex-col items-center gap-3 z-10">
                                <div
                                    className={`flex h-16 w-16 items-center justify-center rounded-xl border border-accent/40 bg-accent/10 text-accent shadow-[0_0_20px_rgba(56,189,248,0.2)] ${isLoading ? "animate-pulse" : ""
                                        }`}
                                >
                                    <i className="bi bi-robot text-2xl" />
                                </div>
                                <span className="font-mono text-[10px] tracking-widest font-bold text-accent/80 uppercase">
                                    Processing
                                </span>
                            </div>
                            ...
                            <div className="flex flex-col items-center gap-3 group">
                                ...
                                <span className="font-mono text-[10px] tracking-widest font-bold text-accent uppercase">
                                    Action
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="mt-10 flex flex-col sm:flex-row items-center justify-between gap-4 relative z-10 w-full">
                            <button
                                onClick={onBack}
                                className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-xl border border-white/10 bg-black/40 px-6 py-4 font-mono text-[11px] uppercase tracking-widest font-bold text-gray-300 transition-all hover:bg-white/5 hover:text-white"
                            >
            ...
            <footer className="flex h-12 w-full items-center justify-between border-t border-white/5 bg-black/20 px-6 backdrop-blur-md relative z-10">
                ...
                <div className="flex items-center gap-6 text-xs text-muted/70 font-mono">
                    <span className="flex gap-2 hidden sm:flex">
